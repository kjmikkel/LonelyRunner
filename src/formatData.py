...
def print_tables(info_list, filename):
	...
	newline = "\\\\\n"
	header = "Speed & Geometrical & Geo Spread & Numerical & Num Spread"
	strAccum = 	"\\begin{tabular}[6]{c|c|c|c|c||c|c|c|c|c}\n" + header + " & " + header + newline + "\hline\n"
	...
	for num in range(0, halflength):
		...
		info = info_list[num]
		first_half = str(info.speed) + " & " + str(info.geo) + " & " + str(info.geo_spread) + " & " + str(info.num) + " & " + str(info.num_spread)		
		if len(info_list) < half_num:		
			strAccum += first_half + " &  & & " + newline
		else:
			info2 = info_list[half_num]
			second_half = str(info2.speed) + " & " + str(info2.geo) + " & " + str(info2.geo_spread) + " & " + str(info2.num) + " & " + str(info2.num_spread)			
