...
\label{detect}
From personal communication with my projects adviser I have been informed of a new method,and its proof, to check whether a configuration makes equation \eqaref{eqa:lonelyRunner} hold. The check can 

If we let $n$ be the number of runners, let $S = \E{s_1, s_2, \ldots, s_n}$ be the set of runner speeds, and $p \in \E{2, \ldots, n+1}$. If there exist a $p$ such that it does not evenly divide any of the speeds in $S$, then equation \eqaref{eqa:lonelyRunner} holds for the set of runner speeds $S$.
