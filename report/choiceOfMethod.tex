...
The advantage of creating the points in advance would be that this is a embarrassingly parallel task, which could be sped up by using modern computers multiple cores. After this the main loop could simply be run without having to dedicate any resources to creating new events.

It would also make the Priority Queue simpler, since instead of a heap, we could simply use a Linked List structure. 
}{
Since all the points are generated at the beginning, this would sharply increase the memory footprint on the system, and if implemented in a programming language without garbage collection, and would require a lot of work.

Also, while the event points themselves could be generated in parallel, they would still have to be inserted into Priority Queue, which would serve as a bottleneck. Even if this was done while all the other event points were still being generated, it would slow down the procedure enormously, loosing a lot of time. The Priority Queue would also still have to be sorted, which in any case would take O$(n\log(n))$, though of course a parallel version of the sort could be used.

If done correctly it would add quite a bit of complexity to the program
}{
...
Compared to creating all the event points up front, only making them when needed would reduce the memory footprint drastically, since new event points would only be made when needed. Doing so would also be much simpler than the parallel version. Since we would only work with relatively few event points in the Priority Queue, insertions and 
}{
Creating each event point sequentially could potentially take a lot more time than the parallel solution.
}{
...
Thus I feel justified in saying that creating the event points on the fly is the preferred solution, as the possible gains by creating all the points at once would be minimal at best, and would unduely increase the complexity of the algorithm. 
...
\Output{The time queue \li, with a new \startT, \eT and \finish inserted for \run as its runner}
