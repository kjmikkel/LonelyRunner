...
\todo{perhaps this should not be so prominently displayed}
...
\begin{algorithm}[H]
  ...
  \ForEach{$d \in \s$}{
    ...
    \ForEach{$d^{\prime} \in \s \setminus \E{d}$}{
      ...
      $start \gets max(\lfloor\frac{k}{d(n+1)}\rfloor, \lfloor\frac{k}{d^{\prime}(n+1)}\rfloor)$

      $stop \gets min(\lceil\frac{nk}{d(n+1)}\rceil, \lceil\frac{nk}{d^{\prime}(n+1)}\rceil)$ 

      \For{$a \leftarrow start$ \KwTo $stop$}{
