...
\begin{algorithm}[H]
  ...
  \SetKwData{no}{No time} 
  \SetKwData{End}{End} \SetKwData{inter}{intersect} \SetKwData{n}{numberOfRunners}
...
\ForEach{t $\in$ \s}{
  $start \gets$ \startT($\frac{1}{t * (\n+1)}$)
  
  $end \gets$ \eT($\frac{1}{t * (\n+1)}$, $\frac{\n}{t * (\n+1)}$, t)
  
  $line \gets$ \finish($\frac{n}{t * (n+1)}$) 
...
\While{\li is not empty}{
  ...
  \If{\p is \eT}{
    ...
    $start \gets$ \startT(\p.time + $\frac{2}{\p.t * (n + 1)}$
  }
