...
My solution to this is to terminate the algorithm if all the runners crosses the start-line at the same time - everything after that will only be a repitition of what had already happened\todo{needs proof}. This also means that the plane sweeping algorithm needs to take care of the case where the runner crosses the start line.
...
At any given time there can at most be $3n + 1$ points in the Priority Queue (specificly when an \comEnd\, has been handeled), and every insertion and removel takes O($\log(n)$) time. However it is clear from the termination proof above, that in the case where the Lonely Runner conjecture does not hold, then the time required is not defined by the number of runners, but rather the smallest common multiple of their speeds, denoted here by $c$. As noted above, in the case where all the numbers are co-prime, $c = s_1 s_2 \cdots s_n$. So in all we have that O($cn\log(n)$) \todo{is this entirely correct?}
}
