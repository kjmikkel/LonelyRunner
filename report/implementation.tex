...
(a + k * (n+1)) * s
 ...
 from equation \eqaref{eqa:integerTime}, where all the symbols has the same meaning as before. The possible large numbers are $a$, (which in the worst case is $n$), $n$ itself and $s$. No matter which built in number type we choose to represent the above expression with, it is clear there exists values for $n$ and $s$ that will cause an arithmetic overflow.
