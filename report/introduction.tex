...
From the last case it is clear that there must exist an integer which makes \eqaref{eqa:lonelyRunner} hold for all the speeds in $R\prime$, so let us now choose one such $q \in \N$, such that for all the speeds in $R\prime$, $x = q + t$ makes them hold for \eqaref{eqa:lonelyRunner}\footnote{Since we are able to cram all our runners into an arbitrary small interval anywhere on $\mathbb{T}$, this must be possible}.
